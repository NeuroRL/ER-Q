...
def visual_plot(df, plot_setting, sim_info, figure_type):
    ...
    for state_sequence, action_sequence in zip(df["states"], df["actions"]):
    ...
    for ext in SAVE_IMAGE_EXT["ext"]:
        plt.savefig(plot_setting["save_figures_dir"] + f"visual.{ext}")
    plt.clf()
    plt.close(fig)
    env._fig = None
