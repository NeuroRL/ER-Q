...
from utils.create_legend_label_utils import create_model_name_for_plot, create_legend_label_list_for_param_plot
...
def param_plot(df, plot_setting, sim_info, figure_type):
    ...
    param_name = sim_info["commom_info"]["cmp_paramname_list"][0]
    ...
    legend_label_list = create_legend_label_list_for_param_plot(sim_info)
    ...
    for each_sim_info in sim_info['each_sim_info_list']:
        if len(each_sim_info["cmp_paramval_list"]) > 1:
            raise InvalidJSONConfigException(f"1パラメータで表現できない変数 (bias_policy_weight等) はparam_plotでは出力できません。指定された変数名: {sim_info["commom_info"]["cmp_paramname_list"]} ")
        else:
            # param_plot_dicの初期化
            param_plot_dict = {
                "param_val": each_sim_info["cmp_paramval_list"][0]
            }
            ...
            if label_for_each_sim_info in param_id_dict_per_label:
                ...
                print(f"{param_id_dict_per_label}は、param_plotでは現在想定していない値です。アルゴリズム名とバイアスポリシーの設定を確認してください。")
    # metrics&代表値毎にplot作成
    for metrics in metrics_list:
        ...
        for summary_statistic in summary_statistic_list:
            ...
            # plot後の設定を指定
            title = ""
            xlabel = convert_paramname_for_plot([param_name])[0]
